...
namespace GoalDigger.DataStore.Databases
{
    public class GoalDiggerDBContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder builder) 
        {
            ...
            // builder.Entity<UserModel>().HasOne(x => x.Feed).WithOne(x => x.User).HasForeignKey<FeedModel>(x => x.uid);
            // builder.Entity<FeedModel>().HasOne(x => x.User).WithOne();
            // builder.Entity<Topping>().HasMany(t => t.PizzaToppings).WithOne(pt => pt.Topping).HasForeignKey(pt => pt.ToppingId);

            builder.Entity<UserModel>().HasMany(x => x.Mentions).WithOne(x => x.User); // change to many-to-many or many-to-one????
